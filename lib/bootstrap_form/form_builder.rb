    ...
    %w{email_field search_field text_field text_area password_field collection_select file_field date_select select}.each do |method_name|
      define_method(method_name) do |name, *args|
        ...
        content_tag :div, class: "control-group#{(' error' if object.errors[name].any?)}"  do
          label("#{name} #{object.class.validators_on(method).map(&:class).include?(ActiveModel::Validations::PresenceValidator) ? '&#42;' : ''}", options[:label], class: 'control-label') +
          content_tag(:div, class: 'controls') do
            ...
            if method_name == 'email_field'
              ...
              content_tag(:div, super(name, *args) + content_tag(:button, content_tag(:i, nil, class:'icon-search icon-white'), id:'search_append_button', class: 'btn btn-inverse', type: 'button', :'data-loading-text'=>'...', title: I18n.t(:search_companies_register)), class: 'input-append') + help
